...
export default function EventDetails() {
  ...
  const load = useCallback(async () => {
    ...
    if (user) {
      ...
      try {
        const myRegs = await axios.get('/api/registrations/me');
        const myReg = myRegs.data.registrations?.find(
          reg => reg.event?._id === id || reg.event === id
        );
        if (myReg) {
          setIsRegistered(true);
          if (myReg.qrCodeDataUrl) setQrCode(myReg.qrCodeDataUrl);
        }
      } catch {
        /* ignore errors */
      }
  ...
  async function register() {
    try {
      const res = await axios.post(`/api/registrations/${id}/register`);
      setQrCode(res.data.registration.qrCodeDataUrl);
      setIsRegistered(true);
      showToast('success', 'Registered! Your QR ticket is ready below.');
      await load();
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {/* QR Code shown after registration */}
          {qrCode && (
            <div className="mt-4 p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
              <p className="font-semibold mb-2 text-slate-800 dark:text-slate-100">🎫 Your QR Ticket</p>
              <img src={qrCode} alt="QR Code" className="mx-auto w-48 h-48" />
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Show this at the event entrance</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
        ...
        {!user && (
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">Please log in to post a review.</p>
        )}
